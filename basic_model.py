...
class G(nn.Module):
    ...
    def gumble_generate(self,x,x_attn):
        ...
        distr = self.model(x, x_attn, target_ids = generate_id, target_attn = torch.ones_like(generate_id).long()).logits
        one_hot = torch.zeros(generate_id.shape[0], generate_id.shape[1],self.tokenizer.vocab_size, device=torch.device('cuda:0'))
    ...
    def add_prefix(self,x,x_attn):
        prefix = self.tokenzied_prefix.repeat(x.shape[0],1)
        x = torch.hstack((prefix,x))
        prefix = self.tokenzied_prefix_attn.repeat(x.shape[0],1)
